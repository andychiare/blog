...
The first thing we need to do is changing the code of `index.js` as follows:
...
We imported the `BrowserRouter` component from `react-router-dom` module and wrapped the `App` component inside it. This enriches the `App` component with routing capabilities.
