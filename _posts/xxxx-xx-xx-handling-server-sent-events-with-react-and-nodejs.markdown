...
Here, we can find the flight arrival timetable that will automatically update when the state of a flight changes. In our demo application, we are going to simulate the flight state changes using scheduled events; however, this does not affect the realism and scope of our implementation.
